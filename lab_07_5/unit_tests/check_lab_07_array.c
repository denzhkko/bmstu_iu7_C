...
// Negative tests
#define NEG_K(PTR_1, PTR_2, ERROR_CODE)             \
int rc;                                             \
int *darr_dst, *darr_dst_end;                       \
rc = key(PTR_1, PTR_2, &darr_dst, &darr_dst_end);   \
ck_assert_int_eq(rc, ERROR_CODE);

START_TEST(neg_k_null_pb)
{
    NEG_K(NULL, &rc, INCORRECT_SRC_PTR);
}
END_TEST

START_TEST(neg_k_null_pe)
{
    NEG_K(&rc, NULL, INCORRECT_SRC_PTR);
}
END_TEST

START_TEST(neg_k_null_pb_lt)
{
    NEG_K(&rc + 1, &rc, INCORRECT_SRC_PTR);
}
END_TEST
...
START_TEST(neg_k_zero_len_filtered_array)
{
    ...
    ck_assert_int_eq(rc, ZERO_F_ARRAY_LEN);
...
Suite *suite_create_key(void)
{
    ...
    tcase_add_test(tcase_neg, neg_k_null_pb);
    tcase_add_test(tcase_neg, neg_k_null_pe);
    tcase_add_test(tcase_neg, neg_k_null_pb_lt);
